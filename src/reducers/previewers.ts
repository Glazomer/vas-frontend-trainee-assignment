...
export type MovePreviewerActionType = 'MOVE';

export type PreviewersActionTypes = 'APPEND' | 'DELETE';
...
export type MovePreviewerAction = {
  type: MovePreviewerActionType;
  step: number;
};
...
const defaultState: PreviewersState = {
  selected: 0,
  previews: [
    {
      alt: 'Sellers image',
...
const reducer = function (
  ...
  switch (action.type) {
    ...
    case 'DELETE': {
      ...
      const newPreviews = previews.filter((_, i) => i !== selected);
    ...
    case 'MOVE': {
      const newSelected = selected + action.step;
    ...
    case 'EDIT': {
      ...
      const newPreviews = previews.map((val, i) =>
        i === selected ? { ...val, [name]: value } : val
      );
